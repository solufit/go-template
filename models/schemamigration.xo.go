...
// SchemaMigration represents a row from 'test.schema_migrations'.
...
func (sm *SchemaMigration) Insert(ctx context.Context, db DB) error {
	...
	// insert (manual)
	const sqlstr = `INSERT INTO test.schema_migrations (` +
		`version` +
		`) VALUES (` +
		`?` +
		`)`
...
func (sm *SchemaMigration) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM test.schema_migrations ` +
		`WHERE version = ?`
	// run
	logf(sqlstr, sm.Version)
	if _, err := db.ExecContext(ctx, sqlstr, sm.Version); err != nil {
...
// SchemaMigrationByVersion retrieves a row from 'test.schema_migrations' as a [SchemaMigration].
//
